...
let getBodyHTMLEmailRemedy = (dataSend) => {
  ...
  const doctorName = dataSend.doctorName || "Không xác định";  // Giá trị mặc định nếu không có tên bác sĩ
  const time = dataSend.timeString || dataSend.time || "Không xác định"; // Giá trị mặc định nếu không có thời gian
  console.log("dataSend:", doctorName);
  console.log("dataSend:", time);
  if (dataSend.language === "vi") {
    result = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; padding: 20px; color: #333;">
        <h2 style="text-align: center; color: #4CAF50;">Bệnh Viện IT</h2>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tr>
                <td style="padding: 10px; border: 1px solid #ddd;">Bác sĩ:</td>
                <td style="padding: 10px; border: 1px solid #ddd;"><strong>${doctorName}</strong></td>
            </tr>
            <tr>
